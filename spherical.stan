...
matrix covariance_spherical(int N, matrix H, real sigmasq, real phi,real nugget) {
matrix[N, N] Sigma;
 ...
 for (i in 1:N) {
      ...
      if (H[i, j] < phi) {
         real scaled_H = H[i, j] / phi;
            Sigma[i, j] = sigmasq * (
              1 - 1.5 * pow(scaled_H, 1) 
                + 0.5 * pow(scaled_H, 3) 
                
            );
      }else {
            Sigma[i, j] = 0;  // Zero covariance if distance >= phi
          }
        Sigma[j, i] = Sigma[i, j];  // Ensure the matrix is symmetric
     }
    }
}
return Sigma;
...
transformed parameters {
  matrix[N, N] Sigma = covariance_spherical(N, H, sigmasq, phi, nugget);
