...
matrix covariance_gaussian(int N, matrix H, real sigmasq, real phi,real nugget) {
matrix[N, N] Sigma;
 ...
 for (i in 1:N) {
      ...
      Sigma[i, j] = sigmasq *(exp(-(H[i,j]/phi)^2));  // Exponential kernel
...
parameters {
  ...
  real<lower=0.00001,upper=50> phi;
   vector<upper = y_cens>[N_cens] ycen;
   real<lower=0> nugget;
 }
 
transformed parameters {
  matrix[N, N] Sigma = covariance_gaussian(N, H, sigmasq, phi, nugget);
